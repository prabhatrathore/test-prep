...
Example: every 5 seconds, your browser sends a request → server checks → responds with new data (if any).
----------------------------------------------------------------------------------

🔹 What is Long Polling?
A smarter version of polling.
Client sends a request → server doesn’t respond immediately.
Instead, server holds the request open until new data arrives.
When data is available, server replies → client immediately sends another request → cycle continues.
...
✅ So it works anywhere — our laptop, server, or cloud — without any issues. 
 */
