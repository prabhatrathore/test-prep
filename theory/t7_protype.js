...
Answer: When we access a property or method of an object, JavaScript looks at the object first. If it’s not found, it checks the object’s prototype, then the prototype’s prototype, and so on, until it reaches Object.prototype or null.
...
When we call a method on an object in JavaScript, the JavaScript engine first looks for the method in the object itself. If the method is not found in the object, it looks for the method in the object's prototype (the object that the current object inherits from). If the method is not found in the prototype, it looks for the method in the prototype's prototype, and so on, until the method is found or there are no more prototypes to search.
...
In this example, both cat and lion objects inherit the type property from the animal object through the prototype chain. And they both share the same describe method defined in the animal object. The hasOwnProperty method can be used to determine if a property is directly defined on an object or if it is inherited through the prototype chain.
...
IndexedDB is a client-side database technology in JavaScript that allows you to store data locally in a user's browser, making it accessible offline. It's a low-level API that provides a more powerful and efficient way of storing data compared to other client-side storage solutions like localStorage and cookies.
IndexedDB provides a way to store large amounts of structured data and enables search and retrieval of data using indices, making it a good choice for applications that need to store large amounts of data and search it quickly. Some use cases of IndexedDB include:
...
ii.ii. Session storage: It stores data for one session and the data is lost when the browser tab is closed.
...
used for transmitting data between a server and a client in web applications. JSON files typically have a .json extension and use the MIME type application/json.
